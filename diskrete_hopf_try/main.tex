...
\prop{
    ...
    \begin{proof}
    Consider the tiny case $m_\R\colon \S(\R\times\R)\rightarrow \S^\R$, specifically
    its fibre: $m_\R^{-1}(0\in\R) = \{~(a,b)\in\S(\R\times\R)~|~|a|^2+|b|^2=1~\wedge~ab^{-1}=0~\}$.
    Find $m_\R^{-1}(0) = \{~(a,b)\in\S(\R\times\R)~|~a=0 \wedge |b|=1~\}=\{0\}\times \{\pm1\}\cong\S^0$.
    Similarly for $t\neq0$ in $\R$ consider: $ab^{-1} = t \Leftrightarrow a = tb \Leftrightarrow b = at^{-1}$.
    It follows $m_\R^{-1}t = \{~ (\pm1,\pm t^{-1}) ~\} \cong \S^0$ for each target in $\R$. Finally consider
    $m_\R^{-1}\infty = \{~(a,b)\in\S(\R\times\R)~|~|a|^2+|b|^2=1~\wedge~ab^{-1}=\infty~\} = \{~(a,0)~\S(\R\times 0) \} \cong \S^0$.

    Similarly find that $m_k^{-1}z \cong \S(k)$ by considering:
    \[ m_k^{-1}z = \{~(a,b)~|~|a|^2+|b|^2=1\wedge ab^{-1}=z~\} = \{~(u,u^{-1}z^{-1})~|~u\in \S(k)~\}.\]
    \end{proof}
}
