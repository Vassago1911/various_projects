...
\prop{
    ...
    Finally consider $p\in \Z\lbrack X \rbrack^\times$. Clearly, since $p$ is invertible in $\Z\lbrack X \rbrack$, it remains
    invertible in $\Z\llbracket X \rrbracket$. By uniqueness of inverses we get $pq=1$ with $q$ another polynomial with integer
    coefficients. 
    \end{proof}
}
